var ISCROLL = function(window, document, Math){
	...
	var utils = (function () {
		...
		me.momentum = function (current, start, time, lowerMargin, wrapperSize, deceleration) {
			var distance = current - start,
				time = time < 100 ? 100 : time, //防止高速滑动，限制最小时间为200ms by lien
				speed = Math.abs(distance) / time,
				destination,
				duration;

			speed = speed > 5 ? 5 : speed; //最大加速度
			...
			/** 
			 * wrapperSize / 8 * ( speed / 8 ) 弹性的大小
			 * wrapperSize / X, X 为高度占比，X 越大占比越小，默认为8
			 */
		...
		// 对低端android加以限定
		me.isBadTransition = /Android /.test(window.navigator.appVersion) && !(/Chrome\/\d/.test(window.navigator.appVersion));
	...
	// INSERT POINT: _init
		...
		_start: function (e) {
			...
			if ( this.options.useTransition && utils.isBadTransition ) {
				time = 0.00001; //低版本手机中对上一次动画进行终止
			}
		...
		_move: function (e) {
			...
			if ( this.options.useTransition && utils.isBadTransition ) {
				this._transitionTime(); //clear 0.001ms，更新移动时间为0，平滑滚动！
			}
