define(function (require, exports, module) {
	return (function (window, document, Math) {
		...
		var utils = (function () {
			...
			var _prefixStyle = commonUtils._prefixStyle;

			self._prefixStyle = _prefixStyle;
			...
			self.momentum = function (current, start, time, lowerMargin, speedLimit, wrapperSize, deceleration) {
			...
			self.isBadTransition = device.os.android && device.os.androidVersion < 4.4 && !device.os.chrome;
			...
			self.hasClass = function (e, c) {
				var re = new RegExp("(^|\\s)" + c + "(\\s|$)");
				return re.test(e.className);
			};

			self.addClass = function (e, c) {
		...
		// INSERT POINT: _init
			...
			/* FOR BAD TRANSITION START: promote */
					...
					if ( utils.isBadTransition ) {
						time = 0.001;

						var pos = this.getComputedPosition();

						this.x = pos.x + this.dropX;
						this.y = pos.y + this.dropY;
		...
		/* REPLACE END: _move */
			...
			_end: function (e) {
				...
				if ( this.options.momentum && duration < 300 ) {
					momentumX = this.hasHorizontalScroll ? utils.momentum(this.x, this.startX, duration, this.maxScrollX, this.options.speedLimit, this.options.bounce ? this.wrapperWidth : 0, this.options.deceleration) : { destination: newX, duration: 0 };
					momentumY = this.hasVerticalScroll ? utils.momentum(this.y, this.startY, duration, this.maxScrollY, this.options.speedLimit, this.options.bounce ? this.wrapperHeight : 0, this.options.deceleration) : { destination: newY, duration: 0 };
		...
		// INSERT POINT: _wheel
			...
			reorderInfinite: function () {
				...
				switch ( this.options.infiniteType ) {
					...
					case "differ":
						...
						// 数据缓冲不足
						if ( this.infiniteCacheLength - this.infinitePhase <= this.options.infiniteCacheBuffer ) {
							this.setup(function (start) {
								this.rearrange(start === 0 ? -1 : 1);
							})
							return;
						}

						// 重新排序;
						this.rearrange(supplement);
						break;
				}

			},

			detectBalance: function () {
				return this.infiniteContentPos[1] + this.infiniteContentPos[0] - 2*this.infiniteContainerCenter;
			},

			rearrange: function (supplement) {
				var item,
					pos,
					size,
					phase,
				...
				switch (supplement) {
					case -1:
						while ( this.detectBalance() < 0 || this.infinitePhase < this.infiniteLength ) {
							// 列表超出最大数据缓存则停止;
							if ( this.infinitePhase >= this.infiniteCacheLength ) return;
							// 当前操作阶段序列
							...
							// 更新滚动区域高度和最大滚动高度
							this.refreshInfinite(phase, size);
						} 
						break;
					case 1:
						while ( this.detectBalance() > 0 ) {
							// 列表序列为0时则停止;
			...
			refreshInfinite: function (phase, size) {
			...
			resizeInfinite: function () {

			},

			updateInfinite: function (item, pos) {
				item._pos = pos;

				if ( this.options.scrollX ) {
					if ( this.options.infiniteUseTransform ) {
						item.style[utils.style.transform] = 'translate(' + pos + 'px, 0)' + this.translateZ;
					} else {
						item.style.left = pos + 'px';
					}
				} else if ( this.options.scrollY ) {
					if ( this.options.infiniteUseTransform ) {
						item.style[utils.style.transform] = 'translate(0, ' + pos + 'px)' + this.translateZ;
					} else {
		...
		function createDefaultScrollbar (direction, interactive, type) {
			...
			if ( type === true ) {
				...
				indicator.style.cssText = 'position: absolute; ' + utils._prefixStyle("box-sizing") + ': border-box; background: rgba(0,0,0,0.4); ' + utils._prefixStyle("border-radius") + ': ' + utils.scale(2.5) + 'px';
			}
